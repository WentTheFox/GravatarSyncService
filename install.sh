...
if [[ ! -f /etc/gravatar-sync/.env ]]; then
    install -Dm600 "$script_dir/.env.example" /etc/gravatar-sync/.env
    echo "wrote /etc/gravatar-sync/.env - edit it before starting the timer"
else
    echo "/etc/gravatar-sync/.env already exists, leaving it alone"
fi

systemctl daemon-reload
echo
echo "installed. next steps:"
echo "  1. edit /etc/gravatar-sync/.env"
